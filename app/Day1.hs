...
toVal :: [Char] -> Int
toVal line =
  let digits = foldl findDigits Nothing line
   in maybe 0 f digits
  where
    f (a, b) = ((digitToInt a) * 10) + digitToInt b

findDigits :: Maybe (Char, Char) -> Char -> Maybe (Char, Char)
findDigits Nothing c
  | isDigit c = Just (c, c)
  | otherwise = Nothing
findDigits (Just (a, b)) c
  | isDigit c = Just (a, c)
  | otherwise = Just (a, b)
