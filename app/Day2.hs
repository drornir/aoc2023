...
toVal :: [Char] -> Int
toVal line =
  let digits = foldl findDigits (Acc line Nothing) line
   in maybe 0 f (val digits)
  where
    f (a, b) = (a * 10) + b
...
findDigits :: Acc -> Char -> Acc
findDigits (Acc l Nothing) c
  | isDigit c = Acc (tail l) (Just (digitToInt c, digitToInt c))
  | otherwise = case startsWithNumString l of
      Just mn -> Acc (tail l) (Just (mn, mn))
      Nothing -> Acc (tail l) Nothing
findDigits (Acc l (Just (a, b))) c
  | isDigit c = Acc (tail l) (Just (a, digitToInt c))
  | otherwise = case startsWithNumString l of
      Just mn -> Acc (tail l) (Just (a, mn))
      Nothing -> Acc (tail l) (Just (a, b))

startsWithNumString :: [Char] -> Maybe (Int)
startsWithNumString s = foldl reducer Nothing $ zip [0 ..] numbers
  where
    reducer :: Maybe (Int) -> (Int, [Char]) -> Maybe (Int)
    reducer Nothing (idx, name)
      | isPrefixOf name s = Just idx
      | otherwise = Nothing
    reducer (Just a) _ = Just a

file =
  [r|tsgbzmgbonethreedrqzbhxjkvcnm3
